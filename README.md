...
| 단계 | 수행 주체 | 영역 | 상세 내용 |
|:---:|:---:|:---:|:---|
| **요청** | Human | PC | 터미널이나 채팅창에서 `/design-api`와 같은 명령어를 입력하여 작업 트리거 |
| **탐색** | AI | PC (Local) | `.claude/commands` 디렉토리에서 해당 커맨드의 설명과 기본 프롬프트 형식을 찾음 |
| **해석** | AI | PC (Local) | `CLAUDE.md` 파일을 읽어 프롬프트에 포함된 약어(`@architecture` 등)를 풀이하고, 필요한 가이드 문서의 URL을 파악 |
| **동기화** | AI | Repo ↔ PC | 중앙 레포지토리에서 최신 가이드(`api-design.md` 등)를 curl로 다운로드하여 로컬 `claude/` 디렉토리에 저장 |
| **추론** | AI | Claude | 조합된 프롬프트(약어 풀이 + 요청 사항)와 가이드 문서의 지침을 결합하여 컨텍스트를 구성 |
| **완료** | AI | Claude → PC | 설계서 작성, 코드 생성 등 기획-설계-개발-배포 단계에 맞는 최종 결과물을 사용자에게 전달 |
