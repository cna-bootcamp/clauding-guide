...
| 단계 | 수행 주체 | 영역 | 상세 내용 |
|:---:|:---:|:---:|:---|
| **요청** | Human | PC | 터미널이나 채팅창에서 `/design-api`와 같은 명령어를 입력하여 작업을 트리거합니다. |
| **탐색** | AI | PC (Local) | `.claude/commands` 디렉토리에서 해당 커맨드의 설명과 기본 프롬프트 형식을 찾습니다. |
| **해석** | AI | PC (Local) | `CLAUDE.md` 파일을 읽어 프롬프트에 포함된 약어(`@architecture` 등)를 풀이하고, 필요한 가이드 문서의 URL을 파악합니다. |
| **동기화** | AI | Repo ↔ PC | 중앙 레포지토리에서 최신 가이드(`api-design.md` 등)를 curl로 다운로드하여 로컬 `claude/` 디렉토리에 저장합니다. |
| **추론** | AI | Claude | 조합된 프롬프트(약어 풀이 + 요청 사항)와 가이드 문서의 지침을 결합하여 컨텍스트를 구성합니다. |
| **완료** | AI | Claude → PC | 설계서 작성, 코드 생성 등 기획-설계-개발-배포 단계에 맞는 최종 결과물을 사용자에게 전달합니다. |
...
전체 생명주기를 완료하였습니다.     
✨ Design Thinking으로 막막한 아이디어를 구체적인 기획으로 다듬었고    
✨ DDD기반으로 복잡한 아키텍처를 설계하며 전체를 조망하는 시야를 길렀고    
✨ Loosely Coupling한 마이크로서비스를 직접 구현해보았고     
✨ 컨테이너와 쿠버네티스라는 현대적 배포의 정수를 익혔고    
✨ CI/CD를 통해 자동화된 잦은 배포로 Fast Fail, Learn And Pivoting하는 진정한 DevOps 철학을 체험했습니다     
