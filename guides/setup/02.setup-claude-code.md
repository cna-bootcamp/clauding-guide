...
아래 명령으로 새로운 스크립트 파일을 생성합니다.         
```
code claude-setup.ps1
```

아래 링크를 브라우저에서 열어 내용을 복사합니다.  그리고 위 파일에 내용을 붙여넣고 저장합니다.    
https://github.com/cna-bootcamp/clauding-guide/blob/main/guides/tools/claude-setup.ps1


아래 명령으로 새로운 스크립트 파일을 생성합니다.    
```
code claude-encoding-fix.ps1
```

아래 링크도 브라우저에서 열어 내용을 복사합니다. 그리고 위 파일에 내용을 붙여넣고 저장합니다.       
https://github.com/cna-bootcamp/clauding-guide/blob/main/guides/tools/claude-encoding-fix.ps1


Powershell 창으로 돌아와 실행합니다.   
```
./claude-setup.ps1
```

현재 창을 열고 새로운 Powershell창을 열어 확인합니다.  
```
claude-help
```

수행 결과가 등록되는 파일을 확인 하려면 아래 명령을 이용하세요.   

```
code $PROFILE
```
아래와 같이 $claudePath가 '.npm-global' 디렉토리로 되어 있어야 합니다.  
![](images/2025-08-04-07-50-29.png)
...
팁) Powershell에서 편의명령어 설정 실패 시 대안.   
Powershell을 띄우고 $PROFILE을 엽니다. 
```
code $PROFILE
```
...
```
저장 후 새로 Powershell창을 열고 cy-yolo로 실행했을 때 YOLO모드로 실행되는 지 확인합니다.   
